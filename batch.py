...
def run_sim(index) :
  ...
  runcmd = 'source /usr/local/geant4_10.6/bin/geant4.sh; source /usr/local/root6/bin/thisroot.sh; ./MdmPpacSim '+'config_stage1~{0}.json'.format(index)
  subprocess.call([runcmd],shell=True)
  ...
  with open("config_stage2~{0}.json".format(index),"w") as outfile :
    json.dump(config,outfile,indent = 4)
  runcmd = 'source /usr/local/geant4_10.6/bin/geant4.sh; source /usr/local/root6/bin/thisroot.sh; ./MdmPpacSim '+'config_stage2~{0}.json'.format(index)
  subprocess.call([runcmd],shell=True)
