...
def run_sim(index) :
  ...
  subprocess.call(["source /usr/local/root6/bin/thisroot.sh;", "source /usr/local/geant4_10.6/bin/geant4.sh;", "./MdmPpacSim","config_stage1~{0}.json".format(index)],shell=True)

  # Stage2
  config["IsTargetChamber"] = False
  with open("config_stage2~{0}.json".format(index),"w") as outfile :
    json.dump(config,outfile,indent = 4)
  subprocess.call(["source /usr/local/root6/bin/thisroot.sh;", "source /usr/local/geant4_10.6/bin/geant4.sh;", "./MdmPpacSim","config_stage2~{0}.json".format(index)],shell=True)
