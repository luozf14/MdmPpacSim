...
namespace MdmPpacSim
{
                                            ...
                                            G4ForceCondition *condition)
  {
    ...
    G4bool test1 = (name == "O18") ? true : false;
    G4bool test2 =
        (aTrack.GetNextVolume()->GetLogicalVolume()->GetName() == "LogicTarget")
            ? true
            : false;
    G4double mfp = DBL_MAX;
    if (test1 && test2 && aTrack.GetPosition().z() < fReactionPosition &&
        aTrack.GetTrackID() == 1)
    { // for target
      mfp = 0;
    }
                                                   ...
                                                   const G4Step &aStep)
  {
    ...
    // TrojanHorse(aTrack, aStep, fTargetCharge, fTargetMass, fLightProductCharge,
    //             fLightProductMass, 0.0, fHeavyProductCharge, fHeavyProductMass,
    //             3.6, 0, 1, 0.0);
  ...
  void ReactionProcess::StartTracking(G4Track *track)
  {
    ...
    fReactionPosition =
        detectorConstruction->GetTargetThicknessInUm() * um /
        std::cos(detectorConstruction->GetTargetRotationAngleInDeg() / 180. *
                 M_PI) *
        G4UniformRand();
  }
  ...
  G4VParticleChange *ReactionProcess::TwoBody(const G4Track &aTrack,
                                              const G4Step &aStep, int Zt, int At,
                                              ...
                                              double Ex1, double Ex2)
  {
    ...
    ////Do some kinematics
    G4double CM_theta = acos(
        -1. + 2. * G4UniformRand());                        // 0->pi relative to the particle direction
    G4double CM_psi = G4UniformRand() * 4. * 2. * atan(1.); // 0->pi
    /// Rotate so theta/psi are relative to the original beam direction
    G4ThreeVector momentumDirection =
        aTrack
            .GetMomentumDirection(); // using rotation method from reactionphysics
    G4ThreeVector v = G4ThreeVector(0., 0., 1.).cross(momentumDirection);
    G4double rotAngle = acos(momentumDirection.z());

    G4ThreeVector dir = G4ThreeVector(sin(CM_theta) * sin(CM_psi),
                                      sin(CM_theta) * cos(CM_psi), cos(CM_theta));
    if (v.getR() > 0)
      dir.rotate(
          v, rotAngle); // rotate the direction to be relative to the beam axis

    // Get Q-value
    //	G4cout<<CM_theta<<G4endl;
    G4double Q_value = 0;
    Q_value = aTrack.GetDynamicParticle()->GetDefinition()->GetPDGMass() +
              targetdef->GetPDGMass() -
              (part1def->GetPDGMass() + part2def->GetPDGMass());
    ...
    G4double p_1 = sqrt(2. * part1->GetMass() * E_cm *
                        (1. * M2 / (M1 + M2))); // E_1 = m2/(m1+m2) * E_t
    G4double p_2 = sqrt(2. * part2->GetMass() * E_cm *
                        (1. * M1 / (M1 + M2))); // E_2 = m1/(m1+m2) * E_t
    ...
    part1->SetKineticEnergy((total_mom_1 * total_mom_1) /
                            (2. * part1->GetMass()));
    part2->SetKineticEnergy((total_mom_2 * total_mom_2) /
                            (2. * part2->GetMass()));
    G4double lab_theta = p_new_1.theta();
    //	aTrack.GetDynamicParticle()->DumpInfo();
    //	G4cout<<"Alpha E:
    //"<<(total_mom_1*total_mom_1)/(2.*part1->GetMass())<<"\tMom. vector:
    //"<<part1->GetMomentumDirection()<<G4endl; 	part1->DumpInfo();
    //	part2->DumpInfo();
    G4Track *sec1 =
        new G4Track(part1, aTrack.GetGlobalTime(), aTrack.GetPosition());

    G4Track *sec2 =
        new G4Track(part2, aTrack.GetGlobalTime(), aTrack.GetPosition());
    // part1->DumpInfo();
    // part2->DumpInfo();
  ...
  G4VParticleChange *
  ReactionProcess::TwoBodyDecay(const G4Track &aTrack, const G4Step &aStep,
                                ...
                                int flag)
  {
    ...
    // calculate in CM1
    G4double mag_Pcm1 = std::sqrt((std::pow(Mcm1, 2) - std::pow(mB + mb, 2)) *
                                  (std::pow(Mcm1, 2) - std::pow(mB - mb, 2))) /
                        2. / Mcm1;
    ...
    G4double Mcm2 = PB.M();
    G4double mag_Pcm2 = std::sqrt((std::pow(Mcm2, 2) - std::pow(mD1 + mD2, 2)) *
                                  (std::pow(Mcm2, 2) - std::pow(mD1 - mD2, 2))) /
                        2. / Mcm2;
    ...
    G4double Mcm3 = PD2.M();
    G4double mag_Pcm3 =
        std::sqrt((std::pow(Mcm3, 2) - std::pow(mD21 + mD22, 2)) *
                  (std::pow(Mcm3, 2) - std::pow(mD21 - mD22, 2))) /
        2. / Mcm3;
    ...
    light->Set4Momentum(
        G4LorentzVector(G4ThreeVector(Pb.Px(), Pb.Py(), Pb.Pz()), Pb.E()));
    heavy->Set4Momentum(
        G4LorentzVector(G4ThreeVector(PB.Px(), PB.Py(), PB.Pz()), PB.E()));
    d1->Set4Momentum(
        G4LorentzVector(G4ThreeVector(PD1.Px(), PD1.Py(), PD1.Pz()), PD1.E()));
    d21->Set4Momentum(G4LorentzVector(
        G4ThreeVector(PD21.Px(), PD21.Py(), PD21.Pz()), PD21.E()));
    d22->Set4Momentum(G4LorentzVector(
        G4ThreeVector(PD22.Px(), PD22.Py(), PD22.Pz()), PD22.E()));

    // set secondary tracks
    G4Track *sec_light =
        new G4Track(light, aTrack.GetGlobalTime(), aTrack.GetPosition());
    G4Track *sec_heavy =
        new G4Track(heavy, aTrack.GetGlobalTime(), aTrack.GetPosition());
    G4Track *sec_d1 =
        new G4Track(d1, aTrack.GetGlobalTime(), aTrack.GetPosition());
    G4Track *sec_d21 =
        new G4Track(d21, aTrack.GetGlobalTime(), aTrack.GetPosition());
    G4Track *sec_d22 =
        new G4Track(d22, aTrack.GetGlobalTime(), aTrack.GetPosition());
  ...
  G4VParticleChange *
  ReactionProcess::TwoBodyAlphaDecay(const G4Track &aTrack, const G4Step &aStep,
                                     int Zt, int At,             // target
                                     int Z1, int A1, double Ex1, // light recoil
                                     int Z2, int A2, double Ex2  // heavy recoil
  )
  {
    ...
    d1->Set4Momentum(
        G4LorentzVector(G4ThreeVector(PD1.Px(), PD1.Py(), PD1.Pz()), PD1.E()));
    d2->Set4Momentum(
        G4LorentzVector(G4ThreeVector(PD2.Px(), PD2.Py(), PD2.Pz()), PD2.E()));
    ...
    G4Track *sec_d1 =
        new G4Track(d1, aTrack.GetGlobalTime(), aTrack.GetPosition());
    G4Track *sec_d2 =
        new G4Track(d2, aTrack.GetGlobalTime(), aTrack.GetPosition());
  ...
  G4VParticleChange *
  ReactionProcess::TwoBodyNeutronDecay(const G4Track &aTrack, const G4Step &aStep,
                                       int Zt, int At,             // target
                                       int Z1, int A1, double Ex1, // light recoil
                                       int Z2, int A2, double Ex2  // heavy recoil
  )
  ...
  G4VParticleChange *
  ReactionProcess::TrojanHorse(const G4Track &aTrack, const G4Step &aStep, int Zt,
                               int At,                     // target
                               int Z1, int A1, double Ex1, // light recoil
                               int Z2, int A2, double Ex2, // heavy recoil
                               int Zs, int As, double Exs) // spectator
  {
    ...
    // define target (target is particle "a")
    G4DynamicParticle *target = new G4DynamicParticle;
    G4ParticleDefinition *targetDef;
    targetDef = G4IonTable::GetIonTable()->GetIon(Zt, At, 0.);
    target->SetDefinition(targetDef);
    ...
    // get beam properties
    G4int ZA =
        aTrack.GetDynamicParticle()->GetParticleDefinition()->GetAtomicNumber();
    G4int AA =
        aTrack.GetDynamicParticle()->GetParticleDefinition()->GetAtomicMass();
    G4double mA =
        aTrack.GetDynamicParticle()->GetParticleDefinition()->GetPDGMass();
    ...
    //
    G4ThreeVector kA(0., 0.,
                     std::sqrt(2. * mA * EA)); // momentum vector of A in lab
    ...
    {
      G4double mag_s =
          G4UniformRand() * 0.1 * ksx; // momentum magnitude of s in anti-lab
    ...
    G4ThreeVector pF_l = kA - ks; // momentum vector of F(x+A) in lab
    // 3.1 Momentum of b and B in CMS of x+A
    G4double Q2 = mx + mA - mb - mB; // Q-value of x+A->b+B
    G4double EbB = ExA + Q2;
    G4double mag_b = std::sqrt(2. * EbB / (1. / mb + 1. / mB));
    ...
    spectator->SetMomentum(ks);
    spectator->SetKineticEnergy(ks.mag2() / (2. * ms));
    ...
    G4Track *sec_heavy =
        new G4Track(heavy, aTrack.GetGlobalTime(), aTrack.GetPosition());
    G4Track *sec_s =
        new G4Track(spectator, aTrack.GetGlobalTime(), aTrack.GetPosition());
  ...
  G4VParticleChange *
  ReactionProcess::TrojanHorseInverse(const G4Track &aTrack, const G4Step &aStep,
                                      int Zt, int At,             // target
                                      int Z1, int A1, double Ex1, // light recoil
                                      int Z2, int A2, double Ex2, // heavy recoil
                                      int Zs, int As, double Exs) // spectator
  {
    ...
    G4ThreeVector kA(
        0., 0., -std::sqrt(2. * mA * EA)); // momentum vector of A in anti-lab
    ...
    {
      G4double mag_s =
          G4UniformRand() * ksx; // momentum magnitude of s in anti-lab
      G4double temp = G4UniformRand() * 0.010;
      while (temp > fWaveFunction->Eval(mag_s, 0, "S"))
      {
        mag_s = G4UniformRand() * ksx;
