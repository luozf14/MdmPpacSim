...
namespace MdmPpacSim
{
    ...
    PrimaryGeneratorAction::PrimaryGeneratorAction(RunAction *runAction, G4bool isTargetChamber, G4int procNum) : fRunAction(runAction), fIsTargetChamber(isTargetChamber), fProcessNum(procNum)
    {
        G4int n_particle = 1;
        ...
        else
        {
            ...
            analysisReader->SetFileName("Stage1~" + std::to_string(fProcessNum) + ".root");
            G4int ntupleId = analysisReader->GetNtuple("Stage1Data");
            analysisReader->SetNtupleIColumn(ntupleId, "SlitBoxTransmitted", fTransmitted);
    ...
    void PrimaryGeneratorAction::GeneratePrimaries(G4Event *anEvent)
    {
        ...
        else
        {
            ...
            fParticleGun->SetParticlePosition(G4ThreeVector(fPositionX * cm, fPositionY * cm, 0.));
            G4double dircX = std::tan(fAngleX / 180. * M_PI);
            G4double dircY = std::tan(fAngleY / 180. * M_PI) * std::sqrt(dircX * dircX + 1.);
            fParticleGun->SetParticleMomentumDirection(G4ThreeVector(dircX, dircY, 1.).unit());
